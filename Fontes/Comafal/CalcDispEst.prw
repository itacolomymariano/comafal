...
User Function CalcDispEst
   ...
   //Implementado Query para checar se o pedido está liberado.
   ...
   //Só apresentar Vendas e/ou Bonificação.
   ...
   */
   If DbSeek(xFilial("SB2")+cProduto+cLocPrd)
      ...
      //nSaldoPrd := SB2->B2_QATU - (SB2->B2_RESERVA + SB2->B2_QPEDVEN) - nQtVend //Five Solutions - 24/03/2010 - Subtrair do saldo a quantidade digitada no próprio pedido.
      ...
      If lConfEtq //Five Solutions Consultoria - 31/03/2010
         //nSaldoPrd := (SB2->B2_QATU) - (SB2->B2_RESERVA + SB2->B2_QPEDVEN)  //Five Solutions - 25/03/2010 - Somar do saldo a quantidade digitada no próprio pedido.
         //nSaldoPrd := (SB2->B2_QATU + nQtVend) - (SB2->B2_RESERVA + SB2->B2_QPEDVEN)  //Five Solutions - 25/03/2010 - Somar do saldo a quantidade digitada no próprio pedido. 31/03/2010 - Só ocorrerem caso de inclusão
         nSaldoPrd := (SB2->B2_QATU + nQtAntPr) - (SB2->B2_RESERVA + SB2->B2_QPEDVEN)  //Five Solutions - 25/03/2010 - Somar do saldo a quantidade digitada no próprio pedido. 31/03/2010 - Só ocorrerem caso de inclusão
      Else 
         If INCLUI
            //nSaldoPrd := (SB2->B2_QATU + nQtVend) - (SB2->B2_RESERVA + SB2->B2_QPEDVEN)  //Five Solutions - 25/03/2010 - Somar do saldo a quantidade digitada no próprio pedido. 31/03/2010 - Só ocorrerem caso de inclusão
      ...
      //MsgInfo("Achei sALDO DO Produto: "+SB2->B2_COD+" no Local "+SB2->B2_LOCAL+" B2_QATU "+Alltrim(Str(SB2->B2_QATU))+" B2_RESERVA "+Alltrim(Str(SB2->B2_RESERVA))+" B2_QPEDVEN "+Alltrim(Str(SB2->B2_QPEDVEN))+" A Qdt Digitada foi "+Alltrim(Str(nQtVend))+" Portando o Sld. Disponivel é: "+Alltrim(Str(nSaldoPrd)))
