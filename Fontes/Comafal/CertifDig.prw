...
±±ºPrograma  ³CERTIFDIG ºAutor  ³Five Solutions      º Data ³  12/09/2011 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Relatorio Grafico para Impressao do Certificado Digital de º±±
±±º          ³ Qualidade.                                                 º±±
...
User Function BrowsCD
   ...
   Private cDimenso := Substr(cDscItem,16,5) //Dimensão do Material
   ...
   */
   ...
   //Relacionamento para pegar Composição Química dos Lotes de Fornecedores
   // ZZ2 - Leitura de Etiquetas
   // SC2 - Ordem de Produção
   // SZ2 - Itens da Ordem de Corte
   // SB8 - Saldos por Lote
   // SZI - Composição Química dos Lotes de Fornecedores
...
nRow		Linha para impressão da imagem
nCol		Coluna para impressão da imagem
...
nRaster	Compatibilidade - Não utilizado
...
*/
      ...
      oPrint:Say  (100,2420,"CERTIFICADO "+cFilialH+" Nº",oFont16)//,,CLR_HBLUE) - Texto do numero do certificado
      ...
      oPrint:Say  (0480,0140,"PEDIDO Nº:",oFont10n)//,,CLR_HBLUE) - Texto PEDIDO Nº:
	  ...
	  oPrint:Say  (0670,1200,"ESPECIFICAÇÃO DO PRODUTO",oFont18)//,,CLR_HBLUE) - ESPECIFICAÇÃO DO PRODUTO
      ...
      oPrint:Say  (0800,0140,"CÓDIGO DO ITEM:",oFont10)//,,CLR_HBLUE) - Texto CÓDITO DO ITEM:
      oPrint:Line (0750,0500,0950, 0500) //coluna apos o codigo do item
      oPrint:Say  (0800,0550,"DESCRIÇÃO DO ITEM:",oFont10)//,,CLR_HBLUE) - Texto DESCRIÇÃO DO ITEM:
      oPrint:Line (0750,1500,0950, 1500) //coluna apos a descricao do item
      oPrint:Line (0825,1500,0825, 2550) //Linha 1 que divide Pesos Teóricos por Metro/Unidade
      oPrint:Line (0875,1500,0875, 2550) //Linha 2 que divide Pesos Teóricos por Metro/Unidade
      
      oPrint:Say  (0830,1600,"Minimo",oFont10)//,,CLR_HBLUE) - Texto Minimo - PESO TEÓRICO (Kg/m)
      oPrint:Say  (0830,1850,"Maximo",oFont10)//,,CLR_HBLUE) - Texto Maximo - PESO TEÓRICO (Kg/m)
      
      oPrint:Say  (0830,2100,"Minimo",oFont10)//,,CLR_HBLUE) - Texto Minimo - PESO TEÓRICO (Kg/Un)
      oPrint:Say  (0830,2400,"Maximo",oFont10)//,,CLR_HBLUE) - Texto Maximo - PESO TEÓRICO (Kg/Un)
      
      oPrint:Line (0825,1775,0950, 1775) //coluna que divide Peso Teóricos por Metro Mínimo/Máximo
      oPrint:Line (0825,2300,0950, 2300) //coluna que divide Peso Teóricos por Unidade Mínimo/Máximo
      
      oPrint:Say  (0770,1550,"PESO TEÓRICO (Kg/m):",oFont10)//,,CLR_HBLUE) - Texto PESO TEÓRICO (Kg/m):
      oPrint:Line (0750,2050,0950, 2050) //coluna apos o peso teórico (Kg/m)
      oPrint:Say  (0770,2100,"PESO TEÓRICO (Kg/Un):",oFont10)//,,CLR_HBLUE) - Texto PESO TEÓRICO (Kg/Un):
      oPrint:Line (0750,2550,0950, 2550) //coluna apos o peso teórico (Kg/Un)
      oPrint:Say  (0770,2600,"PEÇAS/AMARRADO:",oFont10)//,,CLR_HBLUE) - Texto PEÇAS/AMARRADO:

      
      oPrint:Say  (0875,0120,cCodItCrt,oFont12)//,,CLR_HBLUE) - Codigo do Item do Certificado
      oPrint:Say  (0875,0550,cDscItem,oFont12)//,,CLR_HBLUE) - Descrição do Item do Certificado

      oPrint:Say  (0895,1525,Transform(nPsoTMt,"@E 999,999,999.999"),oFont12)//,,CLR_HBLUE) - Peso Teórico Mínimo por Metro (Kg/m)
      oPrint:Say  (0895,2050,Transform(nPsoUnT,"@E 999,999,999.999"),oFont12)//,,CLR_HBLUE) - Peso Teórico Mínimo por Unidade (Kg/Un)
            
      oPrint:Say  (0895,1800,Transform(nPsoNTMt,"@E 999,999,999.999"),oFont12)//,,CLR_HBLUE) - Peso Teórico Máximo por Metro (Kg/m)
      oPrint:Say  (0895,2300,Transform(nPsoNUnT,"@E 999,999,999.999"),oFont12)//,,CLR_HBLUE) - Peso Teórico Máximo por Unidade (Kg/Un)
       
      oPrint:Say  (0875,2750,Transform(nQAmarr,"@E 999,999,999,999"),oFont12)//,,CLR_HBLUE) - Quantidade de Peças do Amarrado

      //Sexto Quadro - Acabamentos - Ensaios Mecânicos - Anaise Visual - Observações
      ...
      oPrint:Say  (1000,0910,"PRODUÇÃO",oFont10)//,,CLR_HBLUE) - Texto PRODUCAO
      
      oPrint:Say  (0980,1230,"DIMENSÕES (mm):",oFont10)//,,CLR_HBLUE) - Texto DIMENSÕES (mm)
      ...
      oPrint:Say  (0970,2285,"OBSERVAÇÕES:",oFont10)//,,CLR_HBLUE) - Texto COMPRIMENTO
      
      oPrint:Say  (1105,0890,cOrdProd,oFont10)//,,CLR_HBLUE) - Numero da Ordem de Producao
      
      //oPrint:Say  (1105,1350,Transform(nDimenso,"@E 999,999,999.999"),oFont10)//,,CLR_HBLUE) - Dimensões
      oPrint:Say  (1105,1350,cDimenso,oFont10)//,,CLR_HBLUE) - Dimensões
      ...
      oPrint:Say  (1170,1070,"ENSAIOS MECÂNICOS",oFont10)//,,CLR_HBLUE) - ENSAIOS MECÂNICOS
      ...
      oPrint:Say  (1310,1615,"TORÇÃO CONFORME'",oFont10)//,,CLR_HBLUE) - Texto TORÇÃO CONFORME
      ...
      //Sétimo Quadro - COMPOSIÇÃO QUÍMICA
	  ...
	  //oPrint:Say  (2105,1200,"COMPOSIÇÃO QUÍMICA",oFont24)//,,CLR_HBLUE) - COMPOSIÇÃO QUÍMICA

	  aCoords := {1605,0103,1679,2999}
      oPrint:FillRect(aCoords,oBrush)    //Tarja cinza
	  oPrint:Say  (1605,1200,"COMPOSIÇÃO QUÍMICA",oFont18)//,,CLR_HBLUE) - COMPOSIÇÃO QUÍMICA
      ...
      oPrint:Line (1680,0590,1780, 0590) //coluna manganês
      ...
      oPrint:Line (1680,2047,1780, 2047) //coluna 2 manganês
      ...
      //Imprime as Composições Químicas dos Lotes de Fornecedores composto neste Produto
      nBoxLin:= 1780
      nLinha := 1790 //2350
      lImp2Col := .T.
      For nCQ := 1 To Len(aCompQuimi)
         ...
         oPrint:Line (nBoxLin,0590,nColCrt, 0590) //coluna manganês
         ...
         oPrint:Line (nBoxLin,2047,n2ColCrt, 2047) //coluna 2 manganês
...
User Function LegSNOE()
   Local aLegenda := {	{"BR_VERDE"		,"Não Emitido" }   ,;
					    {"BR_VERMELHO"  ,"Já Emitido"	  }   }
