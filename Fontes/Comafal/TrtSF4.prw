...
User Function TrtSF4
   ...
   EndIf
   MsgInfo("Atualização SF4, Concluída com Sucesso !!! "+Alltrim(Str(nAtu))+" Registros Atualizados")
Return
Static Function RunSF4
   ...
   While !Eof()
      IncProc("Gerando Sequencial Nat. Operação "+Alltrim(Str(nCont))+" / "+Alltrim(Str(nReg)))
      cCF  := TSF4->F4_CF
      nSeq := 1
