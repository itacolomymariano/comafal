...
±±³Five Solutions³03/01/08³      ³ Descontinuidade de controle por lote   ³±±
±±³              ³               ³ nos produtos tipo PA e PI.             ³±±
...
Private lUsaLocal := .F. //Five Solutions Consultoria(Itacolomy Mariano) - 03/01/2007 - Descontinuar Controle de Localização.
PUBLIC lRodOC := .F. //Five Solutions Consultoria - 31/01/2011
...
If SB1->(dbSeek(xFilial("SB1")+cProduto))
	...
	If lRet
		...
		If !lProc
			...
			EndIf	
			Aviso("ATENCAO !!!","Este Produto NAO tem Estruturas Disponiveis Para Aplicacao em Ordem de Corte. Favor verificar disponibilizações de saldos por lote",{"<< Voltar"})
			lRet := .F.
		EndIf
	EndIf	
...
If Localiza(cProd) .And. lUsaLocal //Five Solutions Consultoria(Itacolomy Mariano) - 03/01/2007 - Descontinuar Controle de Localização.
...
If l650Auto
			...
			If aEmpSli[nX,1] == SC2->C2_PRODUTO
				For nZ := 1 To Len(aEmpBob)
					...
					Else
						...
						//Five Solutions Consultoria(Itacolomy Mariano) - 03/01/2007 - Descontinuar Controle de Localização.
						//If Localiza(aEmpDad[2]) .And. SBF->(dbSeek(xFilial("SBF")+aEmpDad[2]+"01"+aEmpBob[nZ,2]))
						If Localiza(aEmpDad[2]) .And. lUsaLocal .And. SBF->(dbSeek(xFilial("SBF")+aEmpDad[2]+"01"+aEmpBob[nZ,2]))
    ...
    /**************************************
    * Five Solutions Consultoria 31/01/2011
    * 31 de janeiro de 2011
    * Tratamento da variavel l650Auto para permitir exibição da
    * tela de manutenção de empenhos
    ************************************************************/
    lRodOC := .T.
    If MV_PAR13 == 1 //Verifica parâmetro F12 - Mostra Tela de Alteração de Empenhos
       l650Auto := .F.
    EndIf
...
************************************************************/
   If ( Type("lRodOC") # "L" )
      lRodOC := .F.
   EndIf
   l650Auto := If(lRodOC,.T.,.F.)
   /*
   If l650Auto
      MsgInfo("l650Auto é .T.")
   Else
     Alert("l650Auto é .F.")
   EndIf
   */
...
DbSelectArea("SZ3")
If Localiza(SZ3->Z3_PRODUTO) .And. lUsaLocal //Five Solutions Consultoria(Itacolomy Mariano) - 03/01/2007 - Descontinuar Controle de Localização.
...
For nX := 1 To Len(aOps)
	...
	Else
      ...
      * Itacolomy Mariano
      * 03/01/2008 - 16:02 hs
      * Comentario: Comentado devido a descontinuidade da utilização do recurso de
      *             rastreabilidade(Lote) e Localização para produtos acabados(TIPO = 'PA') e Slit(TIPO = 'PI')
      *  		
      *
	  ...
	  * 		If Localiza(SB1->B1_COD) .And. lUsaLocal //Five Solutions Consultoria(Itacolomy Mariano) - 03/01/2007 - Descontinuar Controle de Localização.
	  * 			dbSelectArea("SBF")
	  * 			dbSetOrder(3)
...
* Five Solutions Consultoria
* 11 de março de 2011
* PE utilizado para evitar inconsistência da GETDADOS de inicializar com
* linhas deletadas. Constatar no fonte MATA650.
*************************************************************************/
User Function M650BUT
   ...
   */
   /*
   For nx:=1 to Len(aColsDele)
	  //MsgInfo("Executando M650BUT Item: "+Alltrim(Str(nx))+" Len(aColsDele) "+Alltrim(Str(Len(aColsDele)))+" Len(aColsDele[nx]) "+Alltrim(Str(Len(aColsDele[nx]))))  
	  aColsDele[nx][Len(aColsDele[nx])]:=.F.
   Next nx
   */
   aColsDele:={}
   For nx:=1 to Len(aCols)
	  //MsgInfo("Executando M650BUT Item: "+Alltrim(Str(nx))+" Len(aCols) "+Alltrim(Str(Len(aCols)))+" Len(aCols[nx]) "+Alltrim(Str(Len(aCols[nx])))) 
	  //aCols[nx,15]:=.F.
