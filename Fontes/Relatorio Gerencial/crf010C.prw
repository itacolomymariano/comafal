...
cQuery += " AND D2.D2_TIPO = 'N' AND D2.D2_LOCAL IN ('03','04') AND D2.D_E_L_E_T_ <> '*' "
...
cQuery += " AND SUBSTRING(D2.D2_EMISSAO,1,6) >= '" + Iif((nMes-2 == -1 .and. nMes == 1) .or. (nMes-2 == 0 .and. nMes == 2) ,StrZero(Year(MV_PAR02)-1,4),SUBSTR(DTOS(MV_PAR02),1,4))+ StrZero(Iif(nMes-2 == -1 .and. nMes == 1 ,11,Iif(nMes-2 == 0 .and. nMes == 2 ,12,nMes-2)),2) +"'  " 
cQuery += " AND SUBSTRING(D2.D2_EMISSAO,1,6) <= '" + SUBSTR(DTOS(MV_PAR02),1,6) +"'  "
cQuery += " AND D2.D2_TIPO = 'N'  AND D2.D2_LOCAL IN ('03','04') AND D2.D_E_L_E_T_ <> '*'  AND D2.D2_CF IN (" + Alltrim(cCFOPVdExt) + ") "
...
cQuery += " AND SUBSTRING(D2.D2_EMISSAO,1,6) >= '" + Iif((nMes-2 == -1 .and. nMes == 1) .or. (nMes-2 == 0 .and. nMes == 2) ,StrZero(Year(MV_PAR02)-1,4),SUBSTR(DTOS(MV_PAR02),1,4))+ StrZero(Iif(nMes-2 == -1 .and. nMes == 1 ,11,Iif(nMes-2 == 0 .and. nMes == 2 ,12,nMes-2)),2) +"'  " 
cQuery += " AND SUBSTRING(D2.D2_EMISSAO,1,6) <= '" + SUBSTR(DTOS(MV_PAR02),1,6) +"'  "
cQuery += " AND D2.D2_TIPO = 'N'  AND D2.D2_LOCAL IN ('03','04')  AND D2.D_E_L_E_T_ <> '*'  AND D2.D2_CF IN (" + Alltrim(cCFOPVdInt) + ") "
cQuery += " AND D2.D2_PEDIDO IN (SELECT C5_NUM FROM " 
...
cQuery += " AND D2.D2_LOCAL IN ('03','04') AND D2.D2_CF IN (" + Alltrim(cCFOPVdExt) + ") AND D2.D2_PEDIDO IN (SELECT C5_NUM FROM " 
...
For nX := nMes-2 to nMes
	...
	cQuery += "AND D2.D2_DOC = F2.F2_DOC AND D2.D2_SERIE = F2.F2_SERIE AND D2.D2_LOCAL IN ('03','04') AND D2.D_E_L_E_T_ = ' ' " 
